...
{
    if (variable.type == type_undefined || variable.type == type_none)
        return;
...
char CodeGenerator_includeLibWithOutNamespaceDirect(CodeGenerator *codeGenerator, char *path)
{
    ...
    {
        if (strcmp(fullIncludes[i], path) == 0)
            return 0;
    }
...
void CodeGenerator_generate(CodeGenerator *codeGenerator)
{
    ...
    for (unsigned int i = 0; i < codeGenerator->table.memoryDeclaration.const_num64s; i++)
    {
        ...
        {
            ByteWriter_writeDouble(&byteWriter, declaration.value.value.f);
        }
    ...
    for (unsigned int i = 0; i < codeGenerator->vanguards_size; i++)
    {
        ...
        ByteWriter_writeUInt(&codeGenerator->globalwriter, codeGenerator->vanguardFunctions[codeGenerator->vanguards_size - i - 1].index);
...
unsigned char CodeGenerator_generateStatement(struct CodeGenerator *codeGenerator, struct SyntaxNode node, struct Scope scope, struct ByteWriter *byteWriter)
{
    ...
    case ID_ForNode:
    {
        ...
        {
            if (variableIndex < before || codeGenerator->table.declarations[variableIndex].scope.rgtr == scope_constant)
                continue;
...
unsigned char CodeGenerator_generateExpression(struct CodeGenerator *codeGenerator, struct SyntaxNode node, struct Scope scope, struct ByteWriter *byteWriter)
{
                ...
                if (valueNode.value.type == TT_Identifier)
                {
                    ...
                    if (type == type_function)
                    {
                        ...
                        if (Complex_return.type == TT_Identifier)
                        {
                            ...
                            if ((motherFunction.argumentsCount != variable.argumentsCount) ? (motherFunction.free_arguments && variable.free_arguments) : 0 || variable.returnType != motherFunction.returnType || variable.returnType_isArray != motherFunction.returnType_isArray)
                            {
                    ...
                    if (left.valueNode->value.type == TT_Identifier)
                    {
                        ...
                        if (collection.type == type_Collection)
                        {
                            ...
                            else if (strcmp(fn.identifier.valueNode->value.value.word, "Remove") == 0)
                            {
                                ...
                                default:
                                {
                                    puts("Requires a number as index!");
                                    markTokenError(fn.identifier.valueNode->value);
                                    exit(1);
                                }
                                }
        ...
        case TT_String:
            // They are all constants
            {
                ...
                if (declaration.type == type_undefined)
                {
                    ...
                    {
                    case TT_Int:
                        if (valueNode->value.value.i >= SCHAR_MIN && valueNode->value.value.i <= SCHAR_MAX)
                        { // Byte/Char in C (64 Version)
                            VariableTable_defineVariable(&codeGenerator->table, constant.value, type_byte);
                        }
                        else if (valueNode->value.value.i >= SHRT_MIN && valueNode->value.value.i <= SHRT_MAX)
                        { // 16Bits/Short in C (64 Version)
                            VariableTable_defineVariable(&codeGenerator->table, constant.value, type_short);
                        }
                        else if (valueNode->value.value.i >= INT_MIN && valueNode->value.value.i <= INT_MAX)
                        { // 32Bits/Int in C (64 Version)
                            VariableTable_defineVariable(&codeGenerator->table, constant.value, type_int);
                        }
                        else if (valueNode->value.value.i >= LLONG_MIN && valueNode->value.value.i <= LLONG_MAX)
                        { // 64Bits/Long in C (64 Version)
                        ...
                        break;
                    case TT_Uint:
                        if (valueNode->value.value.u >= 0 && valueNode->value.value.u <= UCHAR_MAX)
                        { // Byte/Char in C (64 Version)
                            VariableTable_defineVariable(&codeGenerator->table, constant.value, type_ubyte);
                        }
                        else if (valueNode->value.value.u >= 0 && valueNode->value.value.u <= USHRT_MAX)
                        { // 16Bits/Short in C (64 Version)
                            VariableTable_defineVariable(&codeGenerator->table, constant.value, type_ushort);
                        }
                        else if (valueNode->value.value.u >= 0 && valueNode->value.value.u <= UINT_MAX)
                        { // 32Bits/Int in C (64 Version)
                            VariableTable_defineVariable(&codeGenerator->table, constant.value, type_uint);
                        }
                        else if (valueNode->value.value.u >= 0 && valueNode->value.value.u <= ULLONG_MAX)
                        { // 64Bits/Long in C (64 Version)
    ...
    case ID_FunctionCallNode:
    {
        ...
        else if (strcmp(name.value.word, "$$get$$") == 0)
        {
            ...
            ByteWriter_writeByte(byteWriter, BC_INPUT_TEXT);
            return type_text;
        }
        ...
        else if (strcmp(name.value.word, "type") == 0)
        {
            ...
            if (functionCallNode.arguments[0].type == ID_ValueNode)
                if (functionCallNode.arguments[0].valueNode->value.type == TT_Identifier)
                {
                    ...
                    type = CodeGenerator_generateExpression(codeGenerator, functionCallNode.arguments[0], scope, &tmpWriter);
            else
                type = CodeGenerator_generateExpression(codeGenerator, functionCallNode.arguments[0], scope, &tmpWriter);
        ...
        else if (strcmp(name.value.word, "GLL_LOAD") == 0)
        {
            ...
            if (libpath.type != ID_ValueNode)
            {
                puts("GLL Library path must be a constant text!");
                markTokenError(name);
                exit(1);
            }

            if (libpath.valueNode->value.type != TT_String)
            {
                puts("GLL Library path must be a constant text!");
                markTokenError(name);
                exit(1);
            ...
            unsigned int libpath_len = strlen(libpath.valueNode->value.value.word);
            ByteWriter_writeUInt(byteWriter, libpath_len);
            for (unsigned int count = 0; count < libpath_len; ++count)
                ByteWriter_writeByte(byteWriter, libpath.valueNode->value.value.word[count]);
